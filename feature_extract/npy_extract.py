...
gallery_path = os.path.join("/ssd/wwz/cv/bishe/lyl_Person_Reid/yolo_detection/bbox_datasets/","train.txt")
pretrained_path = os.path.join("/ssd/wwz/cv/bishe/lyl_Person_Reid/", "Log", "04-11_10-48-09", "net_params.pkl")
...
for line in f:
    ...
    # words = line.split()
    temp_ft = extract_ft(line[0])
    gallery_ft.append([temp_ft, int(line[1]), line[0]])

probe_ft = np.array(probe_ft,dtype=object)
gallery_ft = np.array(gallery_ft,dtype=object)
np.save('/ssd/wwz/cv/bishe/lyl_Person_Reid/feature_extract/probeft-2048.npy',probe_ft)
np.save('/ssd/wwz/cv/bishe/lyl_Person_Reid/feature_extract/galleryft-2048.npy',gallery_ft)